    ...
    public partial class AnimationGenerator : EditorWindow
    {
        ...
        void GenerateAnimation()
        {
            ...
            if (workingInfo != null)
            {
                ...
                //Log.D("The length is" + workingInfo.animator.velocity.magnitude);
                ...
                if (++workingInfo.workingFrame >= workingInfo.info.totalFrame)
                {
                    ...
                    if (generateInfo.Count == 0)
                    {
                        ...
                        SaveAnimationInfo(generatedPrefab);
                ...
                //Log.D(string.Format("Animation '{0}' is Generating. Current frame is {1}", workingInfo.info.animationName, workingInfo.workingFrame));


            }
        }

        [MenuItem("Tools/AnimationInstancing/Animation Generator", false)]
        ...
        private void SaveAnimationInfo(GameObject go)
        {
            var animationInstancing = go.GetComponent<AnimationInstancing>();
            if (animationInstancing == null)
            {
                throw new System.Exception("AnimationInstancing is null");
            }

            var addr = animationInstancing.prefab.ToAddr();
            var folder = Path.GetDirectoryName(addr);

            if (Directory.Exists(folder) == false) Directory.CreateDirectory(folder);
            if (File.Exists(addr)) File.Delete(addr);

            var file = File.Open(addr, FileMode.Create);
                ...
                for (int j = 0; j != frames.Length; ++j)
                {
                    ...
                    if (y + blockHeight > size)
                    {
                        ...
                        if (check)
                        {
                            ...
                            {
                                //Log.E("There is certain animation's frame larger than a texture.");
                                textureCount = 0;
                                return -1;
                            }
                            else
